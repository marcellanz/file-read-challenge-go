...
func (lr *LimitedReader) Read(p []byte) (n int, err error) {
	...
	lr.read += int64(i)
	overshoot := int64(lr.read - lr.limit)
	if overshoot > 0 {
		fmt.Printf("overshoot: %v, read: %v, limit: %v\n", overshoot, lr.read, lr.limit)
		return int(int64(i) - overshoot), io.EOF
	}
	return i, err
}
...
func main192() {
	...
	splitIndex := info.Size() / 2
	{
		file1.Seek(splitIndex, 0)
		c := make([]byte, 1)
		for {
			_, _ = file1.Read(c)
	...
	var lines0 = linesPool.Get().([]string)
	//var lines1 = linesPool.Get().([]string)
	mutex0 := &sync.Mutex{}
	wg := sync.WaitGroup{}
	fwg := sync.WaitGroup{}
	...
	go func() {
		...
		for {
			...
			willScan := scanner0.Scan()
			if len(lines0) == linesChunkLen || !willScan {
				atomic.AddInt64(&fileLineCount, int64(len(lines0)))
				wg.Add(len(lines0))
				linesToProcess := lines0 // bug
				go func() {
